...
routes.get('/downloadMultiple', function (req, res, next) {
    var targets = req.query.items.map(function (filePath) {
    ...
    targets.forEach(file => {
        console.log(`add to zip : ${file}`);
        zip.file(path.basename(file), fs.readFileSync(file));
    });

    var data = zip.generate({base64: false, compression: 'DEFLATE'});

    res.setHeader('Content-disposition', 'attachment; filename=' + req.query.toFilename);
...
routes.post('/copy', function (req, res, next) {
    ...
    targets.forEach(function (target) {
        fs.copyFile(target, path.join(newPath, path.basename(target)), function (err) {
            i -= 1;
            if (err) {
                res.status(500);
                res.send({
